...
for l in L : 
    ...
    #Cumulant Binder 
    plt.plot(temperatures, mean_val, linestyle='-', label = f'L ={l}', color=rainbow_colors[i % len(rainbow_colors)])
    plt.fill_between(temperatures, mean_val - std_val, mean_val + std_val, alpha=0.2, linewidth=2, color=rainbow_colors[i % len(rainbow_colors)])

    i +=1 

plt.xlabel('Temperature (K)')
plt.ylabel('Energy')
plt.title('Relative density fluctuations, K =1' )
plt.legend()
plt.grid(True)
